...
<div class="{classes} grid grid-cols-2 gap-x-2 ">
   ...
   {#if paymentRecord.invoiceNum}
      ...
      <div class="text-left pl-4 pr-2 -indent-4 font-medium"> <a href="/invoices/{paymentRecord.invoiceNum}">{paymentRecord.paymentNotes}</a></div>
   {:else}
      <div class="text-right">Payment notes: </div>
      <div class="text-left pl-4 pr-2 -indent-4 font-medium">{paymentRecord.paymentNotes}</div>
   {/if}
