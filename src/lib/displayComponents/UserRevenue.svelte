...
<div class="flex flex-col sm:flex-row gap-2 bg-primary-contrast-100-900 p-2 rounded-lg {classes}">
   ...
   {#if overdueAmount > 0 && invoices?.length === 1}
      <p class="text-error-100-900">Overdue amount: <a href="/paymentRecords/new?userId={customer.id}&invoiceNum={invoices[0].invoiceNum}">{currencyFormatter.format(overdueAmount)}</a></p>
