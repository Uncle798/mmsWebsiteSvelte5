<script lang="ts">
	...
	} from '$lib/formSchemas/creditCardFormSchema';
	...
	let { form, message, errors, constraints, enhance, delayed, timeout } = superForm(data, {
		...
		onUpdate({ form }) {
			...
			if (form.valid) {
				const { data } = form;
				let date: string =
					data.exp.substring(0, data.exp.indexOf(' ')) +
					data.exp.substring(data.exp.lastIndexOf(' ') + 1);
				const paymentData = {
...
<ExplainerModal modalOpen={explainerModalOpen}>
	{#snippet content()}
...
<form method="POST" use:enhance>
	<div class={classes}>
		<div>
			<label for="billingGivenName" class="label-text"
				>Give name on credit card
				<Tooltip open={givenNameTooltipOpen}>
					<Tooltip.Trigger>
						<Info aria-label="Given name number tool tip" size={15} />
					</Tooltip.Trigger>
					<Tooltip.Content class="card bg-surface-300-700 p-2">Please use any name</Tooltip.Content>
				</Tooltip>
				<input
		...
		<div>
			<label for="billingFamilyName" class="label-text"
				>Family name on credit card
		...
		<div>
			<label for="ccNum" class="label-text"
				>Credit card number
				<Tooltip open={ccToolTipOpen}>
					...
					<Tooltip.Content class="card bg-surface-300-700 p-2">
						Please use 4000000000000002 or any of the card numbers from <a
							href="https://developer.elavon.com/test-cards"
							class="anchor">https://developer.elavon.com/test-cards</a
						>
					</Tooltip.Content>
				</Tooltip>
				<input
		...
		<div class="flex flex-row gap-2">
			<div>
				<label for="ccExp" class="label-text"
					>Expiration
					<Tooltip open={expToolTipOpen}>
						...
						<Tooltip.Content class="card bg-surface-300-700 p-2">
							Please use a future date, from <a
								href="https://developer.elavon.com/test-cards"
								class="anchor">https://developer.elavon.com/test-cards</a
							>
						</Tooltip.Content>
			...
			<div>
				<label for="cvv" class="label-text"
					>CVV code
					<Tooltip open={cvvToolTipOpen}>
						...
						<Tooltip.Content class="card bg-surface-300-700 p-2">
							Please use any valid cvv code, from <a
								href="https://developer.elavon.com/test-cards"
								class="anchor">https://developer.elavon.com/test-cards</a
							>
						</Tooltip.Content>
	...
	{#if !processing}
		<button class="btn preset-filled-primary-50-950 my-1 rounded-lg sm:my-2">{buttonText}</button>
	{:else if processing}
		Processing...
		{#if !delayed && !timeout}
