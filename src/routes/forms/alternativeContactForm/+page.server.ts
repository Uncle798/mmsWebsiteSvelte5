...
export const actions:Actions = {
   default: async (event) => {
      ...
      if(event.locals.user.employee){
         ...
         if(redirectTo){
            redirect(302, `/${redirectTo}?userId=${userId}&leaseId=${leaseId}&addressId=${addressId}&altContactId=${alternativeContact.id}&altAddressId=${altAddress.addressId}&unitNum=${unitNum}`)
         } else {
            return { alternativeContactForm }
         }
      }
