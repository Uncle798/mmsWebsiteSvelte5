	...
	default: async (event) => {
		...
		if (addressForm.valid) {
			let oldAddress = await prisma.address.findFirst({
				where: {
					AND: [{ userId: data.userId }, { softDelete: false }]
				}
			});
			if (oldAddress) {
			...
			let phoneValid;
			if (data.phoneNum1) {
				const phoneValidResponse = await fetch(
					`https://phoneintelligence.abstractapi.com/v1?api_key=${ABSTRACT_API_KEY}&phone=${addressForm.data.phoneNum1Country}${addressForm.data.phoneNum1}`
				);
