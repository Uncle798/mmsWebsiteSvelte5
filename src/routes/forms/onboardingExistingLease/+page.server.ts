...
export const actions: Actions = {
	default: async (event) => {
		...
		const onboardingExistingLeaseForm = await superValidate(
			formData,
			valibot(onboardingExistingLeaseSchema)
		);
		...
		redirect(
			303,
			`/onboarding?leaseId=${lease.leaseId}&userId=${data.customerId}&addressId=${data.addressId}&lien=${data.propertySubjectToLien}`
		);
	}
};
